...
pub async fn list_tags(
    ...
    let tags = db::list_tags(&state.pool, &auth_user.user_id).await?;
...
pub async fn create_tag(
    ...
    let tag = db::create_tag(&state.pool, &req, &auth_user.user_id).await?;
    Ok(Json(tag))
}
...
pub async fn update_tag(
    ...
    let tag = db::update_tag(&state.pool, &id, &req, &auth_user.user_id)
        .await?
...
pub async fn delete_tag(
    ...
    let deleted = db::delete_tag(&state.pool, &id, &auth_user.user_id).await?;
    if !deleted {
...
pub async fn add_tag_to_document(
    ...
    let _ = db::get_tag(&state.pool, &req.tag_id, &auth_user.user_id)
        .await?
        .ok_or(AppError::NotFound)?;
...
pub async fn remove_tag_from_document(
    ...
    let _ = db::get_document(&state.pool, &doc_id, &auth_user.user_id)
        .await?
        .ok_or(AppError::NotFound)?;
